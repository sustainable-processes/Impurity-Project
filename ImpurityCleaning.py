...
from MapRxns import updatecolumns
from rdkit import Chem
from rdkit.Chem import Descriptors


def cleanimpurities(
    analoguerxnsimpfinal,
    analoguerxnsfilt,
    ...
    impquery = imp1.explode(
        "impurities"
    )  # Explode impurity set list per query compound set
    # impquery
    ...
    impfinal["impurityrxn"] = imprxnquery.explode(
        "impurityrxn"
...
def addconditions(row, db):
    ...
    dat = pd.read_sql_query(
        '''SELECT ReactionID,ReagentID,Temperature,Pressure,ResidenceTime,SolventID,CatalystID from ReactionDB Where ReactionID=  "'''
        + str(ID)
        + '''"''',
        db,
    )
...
def checkimpurities(
    ...
    if ncpus > 1:
        ...
        impfinaldis = impfinal
    validimp = impfinaldis.apply(
        valid_impurities,
        hc_prod=hc_prod,
...
def valid_impurities(row, hc_prod, querycompds, reaxys_update=True):
    ...
    if reaxys_update:
        if (
            ...
            and (
                (row.SolventID == "NaN" or row.SolventID is None or not row.SolventID)
                and not row.MissingSolvent
            )
            and (
                (
                    row.CatalystID == "NaN"
                    or row.CatalystID is None
                    or not row.CatalystID
                )
                and not row.MissingCatalyst
            )
            and (row.Temperature is None or not row.Temperature)
            and (row.Pressure is None or not row.Pressure)
        ...
        if (
            (row.ReagentID == "NaN" or row.ReagentID is None or not row.ReagentID)
            and (row.SolventID == "NaN" or row.SolventID is None or not row.SolventID)
            and (
                row.CatalystID == "NaN" or row.CatalystID is None or not row.CatalystID
            )
            and (row.Temperature is None or not row.Temperature)
            and (row.Pressure is None or not row.Pressure)
            and (row.ReactionTime is None or not row.ReactionTime)
