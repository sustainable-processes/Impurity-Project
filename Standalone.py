...
from rdkit.Chem import rdChemReactions, rdPartialCharges, rdmolops  # Reaction processing
...
from MainFunctions import drawReaction, getcompdict, getfragments, initray, mol_with_atom_index, molfromsmiles
...
def balance_rxn_uspto_df(df:pd.DataFrame,IP=IP,**kwargs):
    ...
    dfbal0 = pd.DataFrame(
        data=dfser.tolist(),
        index=dfser.index,
        columns=["rxnsmiles0","Rdata","Pdata", "balrxnsmiles", "msg", "LHSids", "RHSids", "hcrct", "hcprod", "LHSdata", "RHSdata","Rgtdata","Solvdata", "mappedrxn","conf", "msg1"],
    )
...
def balance_rxn_uspto_row(row: pd.Series, IP=IP, **kwargs):
    if kwargs:
        IP={**IP,**kwargs}
    rxnsmiles0,Rdata,Pdata, balrxnsmiles, msg, LHSids, RHSids, hcrct, hcprod, LHSdata, RHSdata,Rgtdata,Solvdata, mappedrxn, conf, msg1 = balance_rxn_dp(rxn=row["uspto_reaction_smiles"], IP=IP)
    return rxnsmiles0,Rdata,Pdata, balrxnsmiles, msg, LHSids, RHSids, hcrct, hcprod, LHSdata, RHSdata,Rgtdata,Solvdata, mappedrxn, conf, msg1
    
def parse_uspto(rxnsmiles:str): #NOTE: MIXTURES NOT WELL REPRESENTED HERE AS REACTION SMILES IS INSUFFICIENT
    reagents=[]
...
def balance_rxn_dp(rxn: str='', IP=IP,rctstorgts=True, **kwargs):
    ...
    Solvdata={}
    Pdata={}
    dicts=["Rdata","Pdata","Rgtdata","Solvdata"] #Allowable inputs, either {} or {..} output of getcompdict
    try: #First try with user inputs
        ...
        if dicts:
            if rxn: #Reaction string is specified
                ...
                output=getspecdat_rxn(rxn, reagents=IP["reagents"], solvents=IP["solvents"],dicts=dicts)
                if "Rdata" in dicts:
                    Rdata=output[0]
                if "Pdata" in dicts:
                    Pdata=output[1]
        ...
        return "Error", "Error", "Error", "Error", msg, [], [], [], [], "Error","Error",{},{},"Error","Error",msg
    # IP["addedspecies"] = [i for i in Rdata]
    rxnsmiles0 = ">>".join(
        [
            getfragments([Rdata[r]["smiles"] for r in Rdata for _ in range(Rdata[r]['count'])], smiles=True),
            getfragments([Pdata[p]["smiles"] for p in Pdata for _ in range(Pdata[p]['count'])], smiles=True),
        ]
    )
    #New
    if "msg" in IP:
        if IP["msg"] and "With hydrogen carriers" in IP["msg"]:
            IP["msg"]="With hydrogen carriers: "+','.join([hcarrier for hcarrier in IP["msg"].split("With hydrogen carriers: ")[1].split(", ")[0].split(",") if hcarrier.isdigit()]) 
        else:
            IP["msg"]=''
    ...
    if mappedrxn != "Error":
        ...
        if ("Unmapped" in msg1 or "unmapped" in msg1) and ("Smiles discrepancy" not in msg1): #Need to skip if mandatory reactants carries over
            if rctstorgts and "unmapped" in msg1: #Adding unmapped mandatory reactants as reagents
                Rgtdata.update({int(ID):copy.deepcopy(Rdata[int(ID)]) for ID in msg1.split('unmapped from LHS: ')[1].split(', ')[0].split(',') if ID.isdigit()})
            ...
            if "mandrcts" in input and input["mandrcts"]: #External mandatory reactants supplied
...
def updaterxns_(row, hc_prod={}):
    ...
    while ("Unmapped" in msg1 or "unmapped" in msg1) or i == 0:  # Unmapped species exist not reflected
...
def fullsmiles(rxnsmiles,Rgtdata={},Solvdata={}):
    ...
    if Rgtdata:
        ...
        fullrxn=rxnlist[0]+'>'+'.'.join([Rgtdata[ID]['smiles'] for ID in Rgtdata for _ in range(Rgtdata[ID]['count']) if Rgtdata[ID]['smiles'] not in rxnlist[0].split('.')])+">"+rxnlist[1]
    return fullrxn
...
def resolvechargebal(LHSdata,RHSdata,refdict,Rdata={},**kwargs): #Only resolves LHS, refdict is Rdata for USPTO and Rgtdata+Rdata for Reaxys
    LHSdata2=copy.deepcopy(LHSdata)
    RHSdata2=copy.deepcopy(RHSdata)
    Rcharge = sum([LHSdata2[ID]["charge"] for ID in LHSdata2 for _ in range(LHSdata2[ID]["count"])])
    Pcharge= sum([RHSdata2[ID]["charge"] for ID in RHSdata2 for _ in range(RHSdata2[ID]["count"])])
    if Rcharge!=Pcharge:
        chargedict={}
        ...
        if len(chargedict)==1:
            ...
            for i in range(minidx,maxidx+1):
                combinations=[c for c in itertools.combinations_with_replacement(list(chargedict.keys()),i) if sum(c)==(Rcharge*-1)]
    #             print(combinations)
                if combinations:
                ...
                if combfound:
                    break       
        if not combfound:
            prodarray=np.array([np.prod(np.array(list(counter_.values()))) for counter_ in combinations_counter])
            idxmin=np.where(prodarray==prodarray.min())
            combinations_counter=[combinations_counter[idxmin_] for idxmin_ in idxmin[0]]
            if len(combinations_counter)>1:
                lenarray=np.array([len(counter_) for counter_ in combinations_counter])
                idxmin2=np.where(lenarray==lenarray.min())
                combinations_counter=[combinations_counter[idxmin_] for idxmin_ in idxmin2[0]]
            if len(combinations_counter)==1:
                print('Combination found')
                combfound=True
        ...
        if chosen_comb:
            for charge in chosen_comb:
                ...
                if len(ccandi)>1:
                    ...
                    LHSdata2[ccandi]['count']=chosen_comb[charge]*LHSdata2[ccandi]['count']
            ...
            return cleavagebal(LHSdata2,RHSdata2,rxnsmiles0,rxn,resolvechargebal=False,Rdata=Rdata,hc_prod={},neutralizecharge=True,**kwargs)
        else:
            raise Exception('No possible candidates available')
def cleavagebal(LHSdata,RHSdata,rxnsmiles,rxn,msg='',Rdata={},Rgtdata={}, Solvdata={},resolvecharge=False,neutralizecharge=False,**kwargs):
    """_summary_
    ...
    for j,rct in enumerate(rxn.GetReactants()):
        unmappedatoms=[atom.GetIdx() for atom in rct.GetAtoms() if not atom.HasProp("molAtomMapNumber")]
        if len(unmappedatoms)==len(rct.GetAtoms()): #Entire reactant is unmapped 
            if msg and 'With hydrogen carriers:' in msg: #CRUCIAL, otherwise infinite loop possible
                hcarriers=[int(ID) for ID in msg.split('With hydrogen carriers: ')[1].split(", ")[0].split(",") if ID.isdigit()]
                if LHSids[j] in hcarriers: #Hydrogen carrier for hydrogen balance..exempt from this function
                    continue
        # print(rxnsmiles)
        z=[unmappedatoms]
        print(z)
            ...
            for subfrag in z:
                ...
                if '.' in candidate:
                    ...
                    for candidate_ in listcandidate:
                        candidate=parsecandidate(candidate_,neutralizecharge=neutralizecharge)
                        candidatelist+=[candidate]
                else:
                    oldcandidatelist+=[candidate]
                    candidate=parsecandidate(candidate,neutralizecharge=neutralizecharge)
                    candidatelist+=[candidate]
        except Exception as e:
            print(e)
    ...
    masterlist=[candidate for candidatelist in finallist for candidate in candidatelist]
    oldmasterlist=[candidateold for candidatelistold in oldfinallist for candidateold in candidatelistold]
    ...
    rxnsmiles02,_,_,balrxnsmiles2,msg0,LHSids2,RHSids2,hcrct2,hcprod2,LHSdata2,RHSdata2,_,_,mappedrxn2,conf2,msg1_0=balance_rxn_dp(Rdata=LHSdata2,Pdata=RHSdata2,Rgtdata=Rgtdata,Solvdata=Solvdata,ignoreH=True,msg=msg,hc_prod={},first=False)
    print(balrxnsmiles2)
    print(LHSdata2)
    print(msg0)
    print(msg1_0)
    ...
    if resolvecharge and "Charge imbalance" in msg0:
        ...
        try:
            rxnsmiles02,balrxnsmiles2,msg0,LHSids2,RHSids2,hcrct2,hcprod2,LHSdata2,RHSdata2,_,_,mappedrxn2,conf2,msg1_0=resolvechargebal(LHSdata2,RHSdata2,Rdata,Rdata=Rdata,ignoreH=True,msg=msg0)
        except Exception as e:
            # raise Exception(e)
            print(str(e))
            pass
    ...
    if addedspecies:
        ...
        if "Already balanced" in msg0:
            msg0=msg0.replace("Already balanced","Balanced with species: "+addedstr)
        elif "Balanced" in msg0:
            msg0=msg0.replace("Balanced","Balanced with species: "+addedstr)
        elif " with species" in msg0:
            msg0=msg0.replace(msg0.split(" with species: ")[1].split(', ')[0],addedstr)
    # Resolving added RHS species
    hclist=[RHSdata2[ID]['formula'] for ID in RHSdata2 if "unmappedfrag" in RHSdata2[ID]]
    if hclist:
    ...
    if removedmandrcts:
        ...
        if "unmapped from LHS" in msg1_0:
            msg1_0=msg1_0.replace(msg1_0.split("unmapped from LHS: ")[1].split(', ')[0],addedstr)
        else:
            msg1_0="Mandatory species unmapped from LHS: "+addedstr+', '+msg1_0
    return rxnsmiles02,balrxnsmiles2,msg0,LHSids2,RHSids2,hcrct2,hcprod2,LHSdata2,RHSdata2,Rgtdata2,Solvdata2,mappedrxn2,conf2,msg1_0
...
def getfragments_dict(refdict,usemapped=True):
    if usemapped:
        ...
        return getfragments([refdict[s]["smiles"] for s in refdict for _ in range(refdict[s]['count'])], smiles=True)


    
def cleavagebal0(LHSdata,RHSdata,balrxnsmiles,mappedsmiles,msg='',Rdata={},Rgtdata={}, Solvdata={},resolvecharge=False,**kwargs):
    """_summary_
...
def getunmappedfrags(mappedsmiles,refdict,specid=0,inst=0,hcarriers=[]):
    ...
    unmappedatoms=[atom.GetIdx() for atom in mappedmol.GetAtoms() if not atom.HasProp("molAtomMapNumber")]
    mappedatoms=[atom.GetIdx() for atom in mappedmol.GetAtoms() if atom.HasProp("molAtomMapNumber")]
    if len(unmappedatoms)==len(mappedmol.GetAtoms()):
        if specid in hcarriers: #Hydrogen carrier for hydrogen balance..exempt from this function
    ...
    if len(nbmap.keys())>1: #Multiple separate fragments
        for nb in nbmap: #Quite inefficient
            ...
            nbs={nb}
            while nbs:
                nbs=[nb__.GetIdx() for nb_ in nbs for nb__ in mappedmol.GetAtomWithIdx(nb_).GetNeighbors() if nb__.GetIdx() in unmappedatoms if nb__.GetIdx() not in atoms_]
                atoms_=atoms_.union(set(nbs))
            unmappedatoms2+=[list(atoms_)]
    else:
        unmappedatoms2+=[unmappedatoms]
    
    if "unmappedatoms" not in refdict[specid] or "mappedatoms" not in refdict[specid] or "fracmapped" not in refdict[specid]:
        refdict[specid].update({"unmappedatoms":{inst:unmappedatoms2},"mappedatoms":{inst:mappedatoms},"fracmapped":{inst:fracmapped}})
    ...
    candidatelist=[]
    rdPartialCharges.ComputeGasteigerCharges(mappedmol) #Compute partial charges in the molecule
    try:
        for nb,subfrag in zip(nbmap,unmappedatoms2):
            candidate_simple=Chem.MolFragmentToSmiles(mappedmol,atomsToUse=subfrag) #Generates a very generic representation of the fragment
            candidate_strict,atomsymbols,_=gen_template_fragment(set(subfrag),mappedmol2,specificity="strict",seoarateAllHs=True,atomsymbols={}) #Generates a strict template fragment, assume mapped mol same as reaction
            charge_m=mappedmol.GetAtomWithIdx(nbmap[nb][2]).GetPropsAsDict()['_GasteigerCharge']
            charge_u=mappedmol.GetAtomWIthIdx(nb).GetPropsAsDict()['_GasteigerCharge']
            if mappedmol.GetAtomWithIdx(nbmap[nb][2]).GetSymbol()==mappedmol.GetAtomWIthIdx(nb).GetSymbol(): #No noticeable difference in electronegativity
                charge_u=0
...
def storeatommap(mappedsmiles, specid=0, idx=0, atommap={}, neighbormap={},inverse=False):
    if isinstance(mappedsmiles,str):
        mappedmol = molfromsmiles(mappedsmiles)
    else:
        mappedmol=mappedsmiles
    for atom in mappedmol.GetAtoms():
        if atom.HasProp("molAtomMapNumber"):
            ...
            if not inverse:
                ...
                atommap[atom.GetIdx()]=(specid,idx,mnum) #NOTE: if aggregating across multiple species, inverse should be False otherwise information is rewritten
            #             breakpoint()
            neighbors = set(
                [
                    nb.GetIdx()
...
def get_strict_smarts_for_atom(atom, numHs_=None, degree_=None, charge_=None,separateAllHs=False):
    """
