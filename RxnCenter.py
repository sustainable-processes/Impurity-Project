...
def reactioncenter(
    ...
    rxncenterser = analoguerxnsassignedfiltdis.apply(
        getrxncenterrow, axis=1, result_type="reduce"
    )
    rxncenterser = pd.Series(
        data=rxncenterser.values, index=rxncenterser.index
    )  # Optional convert modin back to pandas
    rxncenterdf = pd.DataFrame(
        data=rxncenterser.tolist(),
        index=rxncenterser.index,
        columns=["specmap", "rnbmap", "rxncentermapnum", "rxncenter"],
    )
    analoguerxnscent = copy.deepcopy(analoguerxnsassignedfilt)
    analoguerxnscent[
        ["specmap", "rnbmap", "rxncentermapnum", "rxncenter"]
    ] = rxncenterdf
...
def validreactioncenter(analoguerxnscentfilt, ncpus=16, restart=True):
    ...
    validrxncenterser = analoguerxnscentfiltdis.apply(
        validrxncenterrow, axis=1, result_type="reduce"
    )
    validrxncenterser = pd.Series(
        data=validrxncenterser.values, index=validrxncenterser.index
    )
    validrxncenterdf = pd.DataFrame(
        data=validrxncenterser.tolist(),
        index=validrxncenterser.index,
        columns=[
            "LHSdata",
            "msg3",
            "outfrag",
    ...
    analoguerxnsvalid[
        ["LHSdata", "msg3", "outfrag", "outfg", "outneighbor", "unusedanalogue"]
    ] = validrxncenterdf
...
def parsemap(LHSdata, RHSdata):
        ...
        for ridx, mappedsmiles in enumerate(LHSdata[rctid]["mappedsmiles"]):
            if type(mappedsmiles) == tuple:  # Mixture detected
                for ridx2, mappedsmile in enumerate(mappedsmiles):
                    reactantMap, rnbmap = storeatommap(
                        mappedsmile,
                        specid=rctid,
                        idx=(ridx, ridx2),
                        atommap=reactantMap,
        ...
        for pidx, mappedsmiles in enumerate(RHSdata[prodid]["mappedsmiles"]):
            if type(mappedsmiles) == tuple:  # Mixture detected
                for pidx2, mappedsmile in enumerate(mappedsmiles):
                    productMap, pnbmap = storeatommap(
                        mappedsmile,
                        specid=prodid,
                        idx=(pidx, pidx2),
                        atommap=productMap,
    ...
    specmap = {
        mapnum: tuple(
            [elem for map_ in [reactantMap, productMap] for elem in map_[mapnum]]
        )
        for mapnum in reactantMap
    }
...
def validrxncenter(specmap, rxncenter, LHSdata, rnbmap={}):
    ...
    for RC, mapdict in zip(RCs, mapdicts):
        for changemapnum in RC:
            ...
            neidx = [
                atom.GetIdx() for atom in rctmol.GetAtomWithIdx(idxr).GetNeighbors()
            ]
            ...
            for carrierfrag, loc in LHSdata[rctid]["fragloc"][ridx].items():
                for matchidx, corrmatch in enumerate(loc["corrmatches"]):
                    ...
                    if {idxr}.issubset(corrmatch):
                        if loc[
                            "funcgroupids"
                        ]:  # Some carrier fragments don't have functional groups
                            ...
                            # New code for neighbors
                                ...
                                if any(
                                    [
                                        atomidx in loc["funcgroupids"][matchidx]
                                        for atomidx in neidx
                                    ]
                                ):
                                    nb = True
                        else:
    ...
    unusedanalogue = [
        ID
        for ID in LHSdata
        if LHSdata[ID]["formula"] != "H2"
        if (
            "reacfrag" not in LHSdata[ID]
            or set(LHSdata[ID]["reacfrag"].keys()) != set(LHSdata[ID]["fragloc"].keys())
        )
        if ID not in outfrag
        if ID not in outfg
        if ID not in outneighbor
    ]
    #%% New
    addstr = []
    if outfrag:
        addstr.append(
            "Species "
            + ", ".join([str(ID) for ID in outfrag])
            + " not reacting at carrier fragment"
        )
    if outfg:
        addstr.append(
            "Species "
            + ", ".join([str(ID) for ID in outfg])
            + " not reacting at functional group"
        )
    if unusedanalogue:
        addstr.append(
            "Species "
            + ", ".join([str(ID) for ID in unusedanalogue])
