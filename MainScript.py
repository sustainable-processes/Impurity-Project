...
from AnalgRxns import addspeciesdata, filteranaloguerxns, getanaloguerxns
...
# Main Script
masterdbreadpath = (
    "/home/aa2133/Impurity-Project/Reaxys_Data/"  # Master data source path
)
datasources = {
...
masterparams = {
    "folderwritepath": "/home/aa2133/Impurity-Project/Input/",
    "ncpus": 16,
    "showresults": False,
    "writetofile": True,
    "reaxys_update": True,
...
step15params = {"analoguedisplay": 2, "missedanaloguedisplay": 2}
    ...
    **kwargs: Optional user parameters that will replace default parameters in IP
    ...
    if 15 in steps:
        ...
        if impfinalfilt is None:
            impfinalfilt = pd.read_pickle(
                IP["irdir"] + IP["impfinalfiltfilename"] + ".pickle"
            )
        ...
        if impfinalfilt2 is None:
            impfinalfilt2 = pd.read_pickle(
                IP["irdir"] + IP["impfinalfilt2filename"] + ".pickle"
            )
        impfinalfilt3 = IP["impfinalfilt3"]
        if impfinalfilt3 is None:
            impfinalfilt3 = pd.read_pickle(
                IP["irdir"] + IP["impfinalfilt3filename"] + ".pickle"
            )
        impfinalfilt4 = IP["impfinalfilt4"]
        if impfinalfilt4 is None:
            impfinalfilt4 = pd.read_pickle(
                IP["irdir"] + IP["impfinalfilt4filename"] + ".pickle"
            )
        impfinalfilt5 = IP["impfinalfilt5"]
        if impfinalfilt5 is None:
            impfinalfilt5 = pd.read_pickle(
                IP["irdir"] + IP["impfinalfilt5filename"] + ".pickle"
            )
        impfinalfilt["Relevance_morgan"] = impfinalfilt["Relevance_morgan"].apply(
            lambda x: round(x, 2)
        )
        ...
        for i, rxn in enumerate(summary3.rxn):
            ...
            if i > 0 and any(
                [
                    ...
                    and impprod not in [hc_Dict[j]["smiles"] for j in hc_Dict]
                    for impprod in impprods
                ]
            ):
                continue
            display(
                drawReaction(rdChemReactions.ReactionFromSmarts(rxn, useSmiles=True))
            )
            max_relevance = summary3.loc[summary3.rxn == rxn][
            ...
            print(
                "Number of hits: "
                + str(summary3.loc[summary3.rxn == rxn]["Hits_tfiltered"].values[0])
            )
            print(
                "Temperature range (10th to 90th %tile): "
                + "-".join([str(round(t, 1)) for t in Trangei])
            )
            ...
            impframe_old = impfinalfilt.loc[
                impfinalfilt.impurityrxn == rxn
            ...
            for i in range(IP["analoguedisplay"]):
                ...
                display(
                    drawReaction(
                        rdChemReactions.ReactionFromSmarts(
                            frame4.iloc[i].mapped_rxn, useSmiles=True
                        )
                    )
                )
                ...
                print(
                    "Reagents: "
                    + ", ".join(
                        [frame4.iloc[i].NameDict[ID] for ID in rgts if ID not in cats]
                    )
                )
                print(
                    "Catalyst: "
                    + ", ".join([frame4.iloc[i].NameDict[ID] for ID in cats])
                    + ", ".join(frame4.iloc[i].MissingCatalyst)
                )
                print(
                    "Solvents: "
                    + ", ".join([frame4.iloc[i].NameDict[ID] for ID in solvs])
                    + ", ".join(frame4.iloc[i].MissingSolvent)
                )
            ...
            errorcodes = [
                "Invalid condition",
                "Suspect self-reaction (without catalyst or reagent)",
                "Invalid catalyst",
                "Missing reactants/products",
                "Missing temperature",
                "Temperature outside indicated range",
            ]
            for i in range(IP["missedanaloguedisplay"]):
                ...
                if maxrejrel > max_relevance:
                    ...
                    display(
                        drawReaction(
                            rdChemReactions.ReactionFromSmarts(
                                impframe_old.iloc[i].mapped_rxn, useSmiles=True
                            )
                        )
                    )
                    rgts = set(impframe_old.iloc[i].ReagentID) - set(
                        impframe_old.iloc[i].LHS
                    )
                    ...
                    print(
                        ...
                        + ", ".join(
                            [
                                impframe_old.iloc[i].NameDict[ID]
                                for ID in rgts
                                if ID not in cats
                            ]
                        )
                    ...
                    print(
                        "Catalyst: "
                        + ", ".join([impframe_old.iloc[i].NameDict[ID] for ID in cats])
                        + ", ".join(impframe_old.iloc[i].MissingCatalyst)
                    )
                    print(
                        "Solvents: "
                        + ", ".join([impframe_old.iloc[i].NameDict[ID] for ID in solvs])
                        + ", ".join(impframe_old.iloc[i].MissingSolvent)
                    )
...
def step3(IP):
    ...
    if inputquery_analg is None:
        if inputquery is None:
            inputquery = openpickle(IP["dmdir"] + IP["iqfilename"] + ".pickle")
            IP["inputquery"] = inputquery
        inputquery_analg, fragdict = getanaloguespecies(
            inputquery,
            IP["fragdbsource"],
            SQL=IP["SQL"],
            refquery=IP["refquery"],
            ncpus=IP["ncpus"],
            fragtable=IP["fraggroupssource"],
            substancedbsource=IP["substancesource"],
            includefragparents=IP["includefragparents"],
            onlyisotopes=IP["onlyisotopes"],
        )
        if IP["writetofile"]:
    ...
    if IP["SimThresh"] is not None or IP["MWThresh"] is not None:
        if inputquery_analg_updated is None:
            ...
            if IP["MWThresh"] is not None:
                molwt = True
            inputquery_analg_updated = updatequery(
                inputquery_analg,
                fragchoice=IP["fragchoice"],
            ...
            if IP["writetofile"]:
                writepickle(
                    inputquery_analg_updated, IP["dmdir"], IP["iqanalgupdatedfilename"]
                )
            if IP["showresults"]:
    ...
    combinedpooldf = getcombinedpool(
        inputquery_analg_updated,
        fragchoice=IP["fragchoice"],
        ST=IP["SimThresh"],
        fingerprint="morgan",
        morganradius=2,
        MWT=IP["MWThresh"],
        nomixtures=IP["nomixtures"],
        res_format="df",
    )
    combinedpool = getcombinedpool(
        inputquery_analg_updated,
        fragchoice=IP["fragchoice"],
        ST=IP["SimThresh"],
        fingerprint="morgan",
    ...
    if IP["writetofile"]:
        pd.to_pickle(
            combinedpooldf, IP["dmdir"] + IP["combinedpooldffilename"] + ".pickle"
        )
...
def step4(IP):  # MEMORY INTENSIVE
    ...
    analgidsdict, analgrxnsdict, analoguerxnsraw = getanaloguerxns(
        IP["rxnsource"],
        combinedpool,
        combinedpoolex=combinedpoolex,
        workflow=IP["workflow"],
        returnall=IP["returnall"],
    ...
    if IP["writetofile"]:
        ...
        if not analoguerxnsraw.empty:
            pd.to_pickle(
                analoguerxnsraw, IP["dmdir"] + IP["analoguerxnsrawfilename"] + ".pickle"
            )
        pd.to_pickle(
            analgrxnsdict[IP["workflow"]],
...
def step5(IP):  # MEMORY INTENSIVE
    ...
    if analoguerxnsfilt is None:
        ...
        if IP["writetofile"]:
            pd.to_pickle(
                analoguerxnsfilt,
                IP["dpdir"] + IP["analoguerxnsfiltfilename"] + ".pickle",
            )
        if IP["showresults"]:
            IP["analoguerxnsfilt"] = analoguerxnsfilt
    analoguerxns_updated = addspeciesdata(
        analoguerxnsfilt,
        IP["substancesource"],
    ...
    if IP["writetofile"]:
        pd.to_pickle(
            ...
            IP["dpdir"] + IP["analoguerxns_updatedfilename"] + ".pickle",
        )
    if IP["showresults"]:
...
def step6(IP):
    ...
    if analoguerxnsbal is None:
        if analoguerxns_updated is None:
            analoguerxns_updated = pd.read_pickle(
                IP["dpdir"] + IP["analoguerxns_updatedfilename"] + ".pickle"
            )
        ...
        if IP["writetofile"]:
            ...
            pd.to_pickle(
                analoguerxnsbal, IP["dpdir"] + IP["analoguerxnsbalfilename"] + ".pickle"
            )
        if IP["showresults"]:
    ...
    if IP["removehcarriers"]:
        analoguerxnsbalfilt = analoguerxnsbal.loc[
            (~analoguerxnsbal.msg.str.contains(query, case=False, na=False))
            & (~analoguerxnsbal.msg.str.contains("Hydrogen carriers", na=False))
        ]
    else:
        analoguerxnsbalfilt = analoguerxnsbal.loc[
            (~analoguerxnsbal.msg.str.contains(query, case=False, na=False))
    ...
    if IP["writetofile"]:
        pd.to_pickle(
            analoguerxnsbalfilt,
            IP["dpdir"] + IP["analoguerxnsbalfiltfilename"] + ".pickle",
        )
    if IP["showresults"]:
        IP["analoguerxnsbalfilt"] = analoguerxnsbalfilt
    ...
    if analoguerxnsassigned is None:
        if analoguerxnsparsedfilt is None:
            if analoguerxnsparsed is None:
                if analoguerxnsmapped is None:
                    if analoguerxnsbalfilt is None:
                        analoguerxnsbalfilt = pd.read_pickle(
                            IP["dpdir"] + IP["analoguerxnsbalfiltfilename"] + ".pickle"
                        )
                    ...
                    if IP["writetofile"]:
                        pd.to_pickle(
                            analoguerxnsmapped,
                            IP["dpdir"] + IP["analoguerxnsmappedfilename"] + ".pickle",
                        )
                    IP["analoguerxnsmapped"] = analoguerxnsmapped
                analoguerxnsmappedfilt = analoguerxnsmapped.loc[
                    analoguerxnsmapped.mapped_rxn != "Error"
                ]
                ...
                if IP["writetofile"]:
                    pd.to_pickle(
                        analoguerxnsparsed,
                        IP["dpdir"] + IP["analoguerxnsparsedfilename"] + ".pickle",
                    )
                if IP["showresults"]:
                    IP["analoguerxnsparsed"] = analoguerxnsparsed
            if IP["removesmilesdiscrepancy"]:
                analoguerxnsparsedfilt = analoguerxnsparsed.loc[
                    ~analoguerxnsparsed.msg1.str.contains(
                        "discrepancy", case=False, na=False
                    )
                ]
            else:
                analoguerxnsparsedfilt = analoguerxnsparsed
            changedrxns = analoguerxnsparsedfilt.loc[
                (analoguerxnsparsedfilt.msg1.str.contains("Unmapped"))
                | (analoguerxnsparsedfilt.msg1.str.contains("unmapped"))
            ]
            analoguerxns_updated = IP["analoguerxns_updated"]
            if analoguerxns_updated is None:
            ...
            if IP["helpprod"]:
                ...
                hc_prod = {}
            changedrxns = updaterxns(
                changedrxns,
                hc_prod=hc_prod,
                analoguerxns=analoguerxns_updated,
                ncpus=IP["ncpus"],
            )
            analoguerxnsparsedfilt.update(
                changedrxns[
                    [
                        "mapped_rxn",
                        "confidence",
            ...
            if IP["removemappingerror"]:
                analoguerxnsparsedfilt = analoguerxnsparsedfilt.loc[
                    analoguerxnsparsedfilt.mapped_rxn != "Error"
                ]
            if IP["removesmilesdiscrepancy"]:
                analoguerxnsparsedfilt = analoguerxnsparsedfilt.loc[
                    ~analoguerxnsparsedfilt.msg1.str.contains(
                        "discrepancy", case=False, na=False
                    )
                ]
            if IP[
                "removeRHSinsufficient"
            ]:  # WILL REMOVE MOST REACTIONS, limitation of current approach (help compounds insufficient)
                analoguerxnsparsedfilt = analoguerxnsparsedfilt.loc[
                    ~analoguerxnsparsedfilt.msg.str.contains("RHS species insufficient")
                ]
            if IP["nomixtures"]:
                analoguerxnsparsedfilt = analoguerxnsparsedfilt.loc[
                    ~analoguerxnsparsedfilt.msg1.str.contains(
                        "Mixture detected for LHS", case=False, na=False
                    )
                ]
            if IP["removemandatoryunmapped"]:
                analoguerxnsparsedfilt = analoguerxnsparsedfilt.loc[
                    ~analoguerxnsparsedfilt.msg1.str.contains(
                        "Mandatory", case=False, na=False
                    )
                ]
            if IP["writetofile"]:
                pd.to_pickle(
                    analoguerxnsparsedfilt,
                    IP["dpdir"] + IP["analoguerxnsparsedfiltfilename"] + ".pickle",
                )
            if IP["showresults"]:
        ...
        analoguerxnsassigned = assignfrags(
            analoguerxnsparsedfilt, fragdict, ncpus=IP["ncpus"]
        )
        if IP["writetofile"]:
            pd.to_pickle(
                analoguerxnsassigned,
                IP["dpdir"] + IP["analoguerxnsassignedfilename"] + ".pickle",
            )
        if IP["showresults"]:
            IP["analoguerxnsassigned"] = analoguerxnsassigned
    analoguerxnsassignedfilt = analoguerxnsassigned.loc[
        ~(analoguerxnsassigned.msg2.str.contains("not analogue", case=False, na=False))
    ]
    if IP["writetofile"]:
        pd.to_pickle(
...
def step8(IP):
    ...
    if analoguerxnsvalid is None:
        if analoguerxnscentfilt is None:
            if analoguerxnscent is None:
                if analoguerxnsassignedfilt is None:
                    analoguerxnsassignedfilt = pd.read_pickle(
                        IP["dpdir"] + IP["analoguerxnsassignedfiltfilename"] + ".pickle"
                    )
                analoguerxnscent = reactioncenter(
                    analoguerxnsassignedfilt, ncpus=IP["ncpus"]
                )
                if IP["writetofile"]:
                    pd.to_pickle(
                        analoguerxnscent,
                        IP["dpdir"] + IP["analoguerxnscentfilename"] + ".pickle",
                    )
                if IP["showresults"]:
                    IP["analoguerxnscent"] = analoguerxnscent
            analoguerxnscentfilt = analoguerxnscent.loc[
                analoguerxnscent.rxncenter == True
            ]
            if IP["writetofile"]:
                pd.to_pickle(
                    analoguerxnscentfilt,
                    IP["dpdir"] + IP["analoguerxnscentfiltfilename"] + ".pickle",
                )
            if IP["showresults"]:
        ...
        if IP["writetofile"]:
            pd.to_pickle(
                analoguerxnsvalid,
                IP["dpdir"] + IP["analoguerxnsvalidfilename"] + ".pickle",
            )
        if IP["showresults"]:
            IP["analoguerxnsvalid"] = analoguerxnsvalid
    if IP["removeoutfrags"]:
        analoguerxnsfinal = analoguerxnsvalid.loc[
            ~(analoguerxnsvalid["outfrag"].astype(bool))
        ]
    else:
...
def step9(IP):
    ...
    if analoguerxnstempl is None:
        ...
        if IP["writetofile"]:
            pd.to_pickle(
                analoguerxnstempl,
                IP["ipdir"] + IP["analoguerxnstemplfilename"] + ".pickle",
            )
        if IP["showresults"]:
    ...
    if IP["onlyvalidtempl"]:
        ...
        if IP["removefarfg"]:
            analoguerxnstemplfilt = analoguerxnstemplfilt.loc[
                ~analoguerxnstemplfilt.msg4.str.contains(
                    "far away", case=False, na=False
                )
            ]
        if IP["removeunusedprod"]:
            analoguerxnstemplfilt = analoguerxnstemplfilt.loc[
                ~analoguerxnstemplfilt.msg4.str.contains(
                    "not produced", case=False, na=False
                )
            ]
    if IP["writetofile"]:
        pd.to_pickle(
            analoguerxnstemplfilt,
            IP["ipdir"] + IP["analoguerxnstemplfiltfilename"] + ".pickle",
        )
    if IP["showresults"]:
...
def step10(IP):
    ...
    if analoguerxnsimp is None:
        if analoguerxnstemplfilt is None:
            analoguerxnstemplfilt = pd.read_pickle(
                IP["ipdir"] + IP["analoguerxnstemplfiltfilename"] + ".pickle"
            )
        if inputquery is None:
            inputquery = openpickle(IP["dmdir"] + IP["iqfilename"] + ".pickle")
        analoguerxnsimp = applytemplate(
            analoguerxnstemplfilt, inputquery, ncpus=IP["ncpus"]
        )
        if IP["writetofile"]:
            pd.to_pickle(
                analoguerxnsimp, IP["ipdir"] + IP["analoguerxnsimpfilename"] + ".pickle"
            )
        if IP["showresults"]:
            IP["analoguerxnsimp"] = analoguerxnsimp
    analoguerxnsimpfilt = analoguerxnsimp.loc[analoguerxnsimp.msg5 == "Valid"]
    analoguerxnsimpfilt = removeduplicates(
        analoguerxnsimpfilt, ncpus=IP["ncpus"], restart=False
    )
    if IP["writetofile"]:
        pd.to_pickle(
            analoguerxnsimpfilt,
            IP["ipdir"] + IP["analoguerxnsimpfiltfilename"] + ".pickle",
        )
    if IP["showresults"]:
...
def step11(IP):
    ...
    if impfinal is None:
        if analoguerxnsimpfilt is None:
            analoguerxnsimpfilt = pd.read_pickle(
                IP["ipdir"] + IP["analoguerxnsimpfiltfilename"] + ".pickle"
            )
        if analoguerxnsfilt is None:
            analoguerxnsfilt = pd.read_pickle(
                IP["dpdir"] + IP["analoguerxnsfiltfilename"] + ".pickle"
            )
        if analoguerxns_updated is None:
            analoguerxns_updated = pd.read_pickle(
    ...
    impfinalfilt = impfinal.loc[
        (impfinal.msg6 == "Valid")
        | (impfinal.msg6.str.contains("Query"))
        | (impfinal.msg6.str.contains("Self"))
    ]
    if IP["writetofile"]:
...
def step12(IP):
    ...
    if impfinalfilt is None:
        impfinalfilt = pd.read_pickle(
            IP["ipdir"] + IP["impfinalfiltfilename"] + ".pickle"
        )
    if combinedpool is None:
        combinedpool = openpickle(IP["dmdir"] + IP["combinedpoolfilename"] + ".pickle")
    if combinedpoolex is None:
        combinedpoolex = openpickle(
            IP["dmdir"] + IP["combinedpoolexfilename"] + ".pickle"
        )
    if fragdict is None:
        fragdict = openpickle(IP["dmdir"] + IP["fragdictfilename"] + ".pickle")
    ...
    impfinalfilt = relevance_score_morgan(
        impfinalfilt, fragdict, includereagents=True, ncpus=IP["ncpus"]
    )  # setting up morgan fingerprints
    impfinalfilt_rctonly = relevance_score_morgan(
        impfinalfilt0, fragdict, includereagents=False, ncpus=IP["ncpus"]
    )
...
def step13(IP, conditions=[], catalyst=[]):
    ...
    if impfinalfilt5 is None:
        ...
        impfinalfilt2 = conditionfilter(
            impfinalfilt, conditions=conditions
        )  # Remove invalid conditions
        impfinalfilt3 = impfinalfilt2.loc[
            ~impfinalfilt2.msg6.str.contains("Self")
        ]  # Remove suspect self-reactions
        ...
        if IP["writetofile"]:
            pd.to_pickle(
                impfinalfilt2, IP["irdir"] + IP["impfinalfilt2filename"] + ".pickle"
            )
            pd.to_pickle(
                impfinalfilt3, IP["irdir"] + IP["impfinalfilt3filename"] + ".pickle"
            )
            pd.to_pickle(
                impfinalfilt4, IP["irdir"] + IP["impfinalfilt4filename"] + ".pickle"
            )
            pd.to_pickle(
                impfinalfilt5, IP["irdir"] + IP["impfinalfilt5filename"] + ".pickle"
            )
        if IP["showresults"]:
...
def step14(IP, Trange=None):
    ...
    if IP["includefraginfo"]:  # Streamlining needed, slow code
        ...
        fraginfo = pd.DataFrame(
            data=fraginfo.values, index=fraginfo.index, columns=["Fraginfo"]
        )
        impfinalfilt5["Fraginfo"] = fraginfo
        queryfraginfoa = popfragments(inputquery["species"])
        initray(num_cpus=IP["ncpus"])
        impfinalfilt5dis = mpd.DataFrame(impfinalfilt5)
        fragcompare = impfinalfilt5dis.apply(
            fragcomparerow, queryfraginfoa=queryfraginfoa, axis=1, result_type="reduce"
        )
        fragcompare = pd.DataFrame(
            data=fragcompare.values, index=fragcompare.index, columns=["Fragcompare"]
        )
        impfinalfilt5["Fragcompare"] = fragcompare
        impfinalfilt5dis = mpd.DataFrame(impfinalfilt5)
        fraglists = impfinalfilt5dis.apply(summarizefrags, axis=1, result_type="reduce")
        fraglists = pd.DataFrame(
            data=fraglists.tolist(),
            index=fraglists.index,
            columns=[
                "commonfrags1",
                "uniquefragsa1",
                "uniquefragsq1",
    ...
    summary = impfinalfilt5.groupby(["impurityrxn", "querycompds", "impurities"])[
        "impurityrxn"
    ].count()
    ...
    summary["Max_relevance_morg"] = (
        impfinalfilt5.groupby(["impurityrxn", "querycompds", "impurities"])
        .Relevance_morgan.max()
        .round(2)
    )
    summary["Max_relevance_morg_rctonly"] = (
        impfinalfilt5.groupby(["impurityrxn", "querycompds", "impurities"])
        .Relevance_morgan_rctonly.max()
        .round(2)
    )
    summary = summary.sort_values(by=["Max_relevance_morg", "Hits"], ascending=False)
    mainrxn = summary.index.get_level_values(0)[0]

    summary2 = rank_impuritiest(
        summary,
        mainrxn,
        impfinalfilt5,
        trange=Trange,
        relquantile=IP["relquantile"],
        uppertquantile=IP["uppertquantile"],
        lowertquantile=IP["lowertquantile"],
    )
    summary2 = pd.DataFrame(summary2)
    summary3 = summary2.loc[summary2.Hits_tfiltered > 1]
    summary3 = summary3.sort_values(
        by=[
            "Max relevance_tfiltered",
            "Hits_tfiltered",
            "Max relevance_missing",
            "Hits_missing",
        ],
        ascending=False,
    )
    ...
    if IP["showresults"]:
        IP["summary"] = summary
        IP["summary2"] = summary2
        IP["summary3"] = summary3
