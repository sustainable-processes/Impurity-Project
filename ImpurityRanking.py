...
def updatecatalyst(impfinalfilt,catpool,ncpus=16,restart=True):
    ...
    updatedcatalyst=impfinalfiltdis.apply(updatecatalyst_,catpool=catpool,axis=1,result_type='reduce')
    updatedcatalyst=pd.Series(data=updatedcatalyst.values,index=updatedcatalyst.index)
...
def relevance_score_morgan(impfinalfilt,fragdict,version='mean',includereagents=False,expand=1,morganradius=2,ncpus=16,restart=False):
    if ncpus>1:
        if restart:
            initray(num_cpus=ncpus)
        impfinalfiltdis=mpd.DataFrame(impfinalfilt)
        ...
        impfinalfiltdis=impfinalfilt
    relevance_morgan=impfinalfiltdis.apply(relevance_score_morgan_,fragdict=fragdict,version=version,includereagents=includereagents,
                                   expand=expand,morganradius=morganradius,axis=1,result_type='reduce')
    relevance_morgan=pd.Series(data=relevance_morgan.values,index=relevance_morgan.index)
    impfinalfilt['Relevance_morgan']=relevance_morgan
    return impfinalfilt


def relevance_score_morgan_(row,fragdict,version='mean',includereagents=False,expand=1,morganradius=2): #Will use morgan/circular fingerprints and dice similarity
#     breakpoint()
...
#     breakpoint()
    ...
    if includereagents and Rgtdata:
        ...
        for rgtid in rgtids:
            ...
            fp1 = AllChem.GetMorganFingerprint(m1,morganradius)
            frags=set(getCarrierFrags0(m1,userinput='mol',expand=expand,resFormat='smiles')).intersection(set(fragdict.keys()))
            if not frags:
                frags=[frag for frag in fragdict if rgtid in fragdict[frag]['analoguepool']]
            if frags:
                querycompds=[]
                for frag in frags:
...
def standardize(impfinalfilt,reaxys_update=True,ncpus=16,restart=False):
    ...
    standardized=pd.DataFrame(data=standardized.tolist(),index=standardized.index,columns=['querycompds','impurities','impurityrxn'])
    impfinalfilt[['querycompds','impurities','impurityrxn']]=standardized
    if reaxys_update:
        impfinalfilt=impfinalfilt.reset_index().drop_duplicates(subset=['Instance','ReactionID','impurityrxn']).set_index(['ReactionID','Instance'])
    else:
        impfinalfilt=impfinalfilt.reset_index().drop_duplicates(subset=['ReactionID','impurityrxn']).set_index('ReactionID')
    return impfinalfilt
    ...
    for expand in expandfrag:
        ...
        #     fragdat.update({queryspec:{'frag0':frag0,'frag0idx':frag0idx}})
            ...
            for i,fragsmiles in enumerate(fragdat):
                ...
                corr_matches,funcids,_=get_matches(m,pattmol,checkresults=False) #Chem.MolFromSmarts(Chem.MolToSmarts(Chem.MolFromSmiles(fragsmiles,sanitize=False)))
                if not corr_matches:
                    continue
                        ...
                        for j,func in enumerate(funcids):
                            ...
                            if func1!=func:
                                ...
                                for funcid_1 in list(fragdat1.values()):
                                    if not extra:
                                        break
                                    for k,funcid__1 in enumerate(funcid_1['funcgroupids']):
                                        if not extra:
                                            break
                                        inters=funcid__1.intersection(extra)
                                        if inters:
                                            corr_matches[j]=tuple(set(corr_matches[j]).union(set(funcid_1['corrmatches'][k])))
                                            molfrag=Chem.MolFragmentToSmiles(m,corr_matches[j])
                                            extra=extra-inters

#                 corr_matches=[tuple((idx for idx in co if idx<len(molfromsmiles(queryspec).GetAtoms()))) for co in corr_matches]
                if not molfrag:
                    fragloc[queryspec].update(OrderedDict({fragsmiles:{'corrmatches':corr_matches,'funcgroupids':funcids}}))
                else:
                    fragloc[queryspec].update(OrderedDict({molfrag:{'corrmatches':corr_matches,'funcgroupids':funcids}}))
...
def fragcomparerow(row,queryfraginfoa,includefp=True):
    ...
    fragcompare=OrderedDict({expand:OrderedDict(OrderedDict()) for expand in queryfraginfoa.keys()})
...
#                 breakpoint()
                ...
                commonfrag=set(queryfraginfo[query].keys()).intersection(set(fraginfo[analg].keys()))
                if not commonfrag:
                    for k in range(len(fraginfo.keys())):
                        ...
                        if commonfrag:
                            break
                uniquefragsa=set(fraginfo[analg].keys())-set(queryfraginfo[query].keys())
                uniquefragsq=set(queryfraginfo[query].keys())-set(fraginfo[analg].keys())
                # Fingerprints
                if includefp:
                ...
                for expand in queryfraginfoa.keys():
                    if query not in fragcompare[expand]:
                        fragcompare[expand].update(OrderedDict({query:{analg:{'role':role,'morgansim':round(simm,2),
                                                              'topsim':round(simt,2)}}}))
                    else:
                        fragcompare[expand][query].update({analg:{'role':role,'morgansim':round(simm,2),'topsim':round(simt,2)}})
                expand=list(queryfraginfoa.keys())[i]
                fragcompare[expand][query][analg].update({'commonfrags':list(commonfrag),'uniquefragsa':list(uniquefragsa),
...
def summarizefrags(row):
    ...
    commonfrags1=[comfrag for query in fragcompare['fraginfo1'] for frag in fragcompare['fraginfo1'][query] 
                  for comfrag in fragcompare['fraginfo1'][query][frag]['commonfrags']]
    uniquefragsa1=[uniquefraga for query in fragcompare['fraginfo1'] for frag in fragcompare['fraginfo1'][query] 
                  for uniquefraga in fragcompare['fraginfo1'][query][frag]['uniquefragsa']]
    uniquefragsq1=[uniquefragq for query in fragcompare['fraginfo1'] for frag in fragcompare['fraginfo1'][query] 
                  for uniquefragq in fragcompare['fraginfo1'][query][frag]['uniquefragsq']]
    
    commonfrags2=[comfrag for query in fragcompare['fraginfo2'] for frag in fragcompare['fraginfo2'][query] 
                  for comfrag in fragcompare['fraginfo2'][query][frag]['commonfrags']]
    uniquefragsa2=[uniquefraga for query in fragcompare['fraginfo2'] for frag in fragcompare['fraginfo2'][query] 
                  for uniquefraga in fragcompare['fraginfo2'][query][frag]['uniquefragsa']]
    uniquefragsq2=[uniquefragq for query in fragcompare['fraginfo2'] for frag in fragcompare['fraginfo2'][query] 
                  for uniquefragq in fragcompare['fraginfo2'][query][frag]['uniquefragsq']]
    
    commonfrags3=[comfrag for query in fragcompare['fraginfo3'] for frag in fragcompare['fraginfo3'][query] 
                  for comfrag in fragcompare['fraginfo3'][query][frag]['commonfrags']]
    uniquefragsa3=[uniquefraga for query in fragcompare['fraginfo3'] for frag in fragcompare['fraginfo3'][query] 
                  for uniquefraga in fragcompare['fraginfo3'][query][frag]['uniquefragsa']]
    uniquefragsq3=[uniquefragq for query in fragcompare['fraginfo3'] for frag in fragcompare['fraginfo3'][query] 
                  for uniquefragq in fragcompare['fraginfo3'][query][frag]['uniquefragsq']]
    
    return commonfrags1,uniquefragsa1,uniquefragsq1,commonfrags2,uniquefragsa2,uniquefragsq2,commonfrags3,uniquefragsa3,uniquefragsq3
    


### Ranking ###

def rank_impuritiest(summary,mainrxn,impfinalfilt,trange=None,relquantile=0.75,uppertquantile=0.9,lowertquantile=0.1,process='explode'):
#     breakpoint()
    summarytglobal=[]
    summaryt={}
    ...
    print('Only '+str(round((len(mainframe2.index)/len(mainframe.index))*100,-1))+'% have temperature records (main reaction)')
    ...
    mainframe['Relevance_morgan']=mainframe['Relevance_morgan'].apply(lambda x: round(x,2))
    mainframe2['Relevance_morgan']=mainframe2['Relevance_morgan'].apply(lambda x: round(x,2))
    mainframe3['Relevance_morgan']=mainframe3['Relevance_morgan'].apply(lambda x: round(x,2))
    if process=='explode':
        thresh=mainframe3.Relevance_morgan.quantile(relquantile)
    else:
        thresh=mainframe2.Relevance_morgan.quantile(relquantile)
    lowerbound=mainframe3.loc[mainframe3.Relevance_morgan>=thresh].Temperature.quantile(lowertquantile)
    lowerbound=float(floor(lowerbound))
    upperbound=mainframe3.loc[mainframe3.Relevance_morgan>=thresh].Temperature.quantile(uppertquantile)
...
#     mainframe4=mainframe3.loc[(mainframe3.Temperature<=upperbound) & (mainframe3.Temperature>=lowerbound) & (mainframe3.Relevance_morgan>=thresh)].sort_values(by='Relevance_morgan',ascending=False)
    mainframe4=mainframe3.loc[(mainframe3.Temperature<=upperbound) & (mainframe3.Temperature>=lowerbound)].sort_values(by='Relevance_morgan',ascending=False)
    max_relevance=mainframe4.Relevance_morgan.max()
    rejrxns=mainframe.loc[(~mainframe.Temperature.astype(bool)) & (mainframe.Relevance_morgan>max_relevance)]
    rejrxns.sort_values(by='Relevance_morgan',ascending=False)
    summaryt.update({'rxn':mainrxn,'querycompds':mainframe3.iloc[0].querycompds,
                     'products':mainframe3.iloc[0].impurities,'Frame':mainframe,'Hits_old':len(mainframe.index),
                     'Max relevance_old':mainframe.Relevance_morgan.max(),
                     'Frame2':mainframe2,'tindicated (%)':round((len(mainframe2.index)/len(mainframe.index))*100,-1),
                     'Hits_tonly':len(mainframe2.index),'Max relevance_tonly':mainframe2.Relevance_morgan.max(),
                      'Frame3':mainframe3,'thistogram':fig,'tbarfreq':barcount,
                     'tbarrel':barrel,'Hits_tonlyexp':len(mainframe3.index),'Relevance threshold':thresh,
                     't_range':t_range,'Frame4':mainframe4,
                     'Max relevance_tfiltered':max_relevance,'Hits_tfiltered':len(mainframe4.index),
                    'Reactions_missing':rejrxns,'Max relevance_missing':rejrxns.Relevance_morgan.max(),
...
#         breakpoint()
        ...
        if tindicated!=0: #No temperature entries
            ...
            if maxtempi>maxtemp:
                upperbins=list(range(maxtemp+binwidth,maxtempi+binwidth,binwidth))
            bins=lowerbins+refbins+upperbins
            fig=constructhist(mainframe3.Temperature,bins,'Main product range',col2=impframe3.Temperature,label2='Impurity range')
            barcount,barrel=constructbar(mainframe3,bins,'Main product range',impframe=impframe3,label2='Impurity range')
        else:
            fig=None
            barcount=None
            barrel=None
        ...
        impframe['Relevance_morgan']=impframe['Relevance_morgan'].apply(lambda x: round(x,2))
        impframe2['Relevance_morgan']=impframe2['Relevance_morgan'].apply(lambda x: round(x,2))
        impframe3['Relevance_morgan']=impframe3['Relevance_morgan'].apply(lambda x: round(x,2))
        if process=='explode':
            thresh=impframe3.Relevance_morgan.quantile(relquantile)
        else:
            thresh=impframe2.Relevance_morgan.quantile(relquantile)
        lowerboundi=impframe3.loc[impframe3.Relevance_morgan>=thresh].Temperature.quantile(lowertquantile)
        upperboundi=impframe3.loc[impframe3.Relevance_morgan>=thresh].Temperature.quantile(uppertquantile)
        try:
...
#         impframe4=impframe3.loc[(impframe3.Temperature<=upperbound) & (impframe3.Temperature>=lowerbound) & (impframe3.Relevance_morgan>=thresh)].sort_values(by='Relevance_morgan',ascending=False)
        impframe4=impframe3.loc[(impframe3.Temperature<=upperbound) & (impframe3.Temperature>=lowerbound)].sort_values(by='Relevance_morgan',ascending=False)
        max_relevance=impframe4.Relevance_morgan.max()
        rejrxns=impframe.loc[(~impframe.Temperature.astype(bool)) & (impframe.Relevance_morgan>max_relevance)]
        rejrxns.sort_values(by='Relevance_morgan',ascending=False)
        summaryt.update({'rxn':imprxn,'querycompds':impframe.iloc[0].querycompds,
                         'products':impframe.iloc[0].impurities,'Frame':impframe,'Hits_old':len(impframe.index),
                         'Max relevance_old':impframe.Relevance_morgan.max(),
                         'Frame2':impframe2,'tindicated (%)':tindicated,
                         'Hits_tonly':len(impframe2.index),'Max relevance_tonly':impframe2.Relevance_morgan.max(),
                          'Frame3':impframe3,'thistogram':fig,'tbarfreq':barcount,
                         'tbarrel':barrel,'Hits_tonlyexp':len(impframe3.index),'Relevance threshold':thresh,
                         't_range':t_rangei,'Frame4':impframe4,
                         'Max relevance_tfiltered':max_relevance,'Hits_tfiltered':len(impframe4.index),
                        'Reactions_missing':rejrxns,'Max relevance_missing':rejrxns.Relevance_morgan.max(),
                         'Hits_missing':len(rejrxns.index)})
        summarytglobal+=[summaryt] 
        plt.close('all')
...
def constructhist(col,bins,label,col2=None,label2=None):
    fig=plt.figure()
    hist_1,bins1,p=plt.hist(col,bins=bins,weights=np.zeros_like(col) + 1. /col.size,label=label,alpha=0.5)
    if col2 is not None:
        hist_2,bins2,p2=plt.hist(col2,bins=bins,weights=np.zeros_like(col2) + 1. /col2.size,label=label2,alpha=0.5)
...
def constructbar(mainframe,bins,label,impframe=None,label2=None,binwidth=10):
    ...
    for refb in bins:
        plotbins+=[str(refb)+ '\n'+' to '+'\n'+str(refb+binwidth)]
        maxrelm=mainframe.loc[(mainframe.Temperature<refb+binwidth) & (mainframe.Temperature>=refb)].Relevance_morgan.max()
        totcountm=mainframe.loc[(mainframe.Temperature<refb+binwidth) & (mainframe.Temperature>=refb)].size
        if np.isnan(maxrelm):
            maxrelm=0
        relm+=[maxrelm]
        countm+=[totcountm]
        if impframe is not None:
            maxreli=impframe.loc[(impframe.Temperature<refb+binwidth) & (impframe.Temperature>=refb)].Relevance_morgan.max()
            totcounti=impframe.loc[(impframe.Temperature<refb+binwidth) & (impframe.Temperature>=refb)].size
            if np.isnan(maxreli):
                maxreli=0
            reli+=[maxreli]
            counti+=[totcounti]
