...
from collections import (
    OrderedDict,
)  # Preserve order of keys relative to reaction from left to right
...
def checkrxnrow(row):
    return checkrxn(
        mappedrxn=row["mapped_rxn"], Rdata=row["LHSdata"], Pdata=row["RHSdata"]
    )


def checkrxn(
    mappedrxn, LHSdata=OrderedDict({}), RHSdata=OrderedDict({}), Rdata=None, Pdata=None
    ...
    rmixtures = [
        ID for ID in Rdata if len(Rdata[ID]["smiles"].split(".")) > 1
    ]  # sUSPECT
    pmixtures = [ID for ID in Pdata if len(Pdata[ID]["smiles"].split(".")) > 1]
    msg = ""
    if rmixtures:
        msg += (
            "LHS species " + ", ".join([str(ID) for ID in rmixtures]) + " is a mixture"
        )
    if pmixtures:
        if msg:
            msg = (
        ...
        for ID0 in Rdata:
            if Rdata[ID0]["smiles"] == rctsmiles and ID0 in unusedrct:
                ...
                if ID0 in LHSdata.keys():
                    LHSdata[ID0]["mappedsmiles"].extend(
                        [Chem.MolToSmiles(rdrxn.GetReactants()[ID])]
                    )
                    LHSdata[ID0]["cleanmol"].extend([rct])
                else:
                    LHSdata.update({ID0: Rdata[ID0]})
                    LHSdata[ID0].update(
                        {"mappedsmiles": [Chem.MolToSmiles(rdrxn.GetReactants()[ID])]}
                    )
        ...
        for ID0 in Pdata:
            if Pdata[ID0]["smiles"] == prodsmiles and ID0 in unusedprod:
                ...
                if ID0 in RHSdata.keys():
                    RHSdata[ID0]["mappedsmiles"].extend(
                        [Chem.MolToSmiles(rdrxn.GetProducts()[ID])]
                    )
                    RHSdata[ID0]["cleanmol"].extend([prod])
                else:
                    RHSdata.update({ID0: Pdata[ID0]})
                    RHSdata[ID0].update(
                        {"mappedsmiles": [Chem.MolToSmiles(rdrxn.GetProducts()[ID])]}
                    )
...
def assignfrags(LHSdata, querydict, expand=1):
    ...
    for rctid in LHSdata:
        analgfrags = getCarrierFrags0(
            fragdata[rctid]["smiles"], expand=expand, resFormat="smiles"
        )
        ...
        fragdata[rctid]["querycompds"].update(
            {fragsmiles: querydict[fragsmiles] for fragsmiles in commonfrags}
        )
    if nonanalogue:
        msg = (
            "Species "
